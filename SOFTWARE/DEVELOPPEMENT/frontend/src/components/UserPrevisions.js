...
function UserPrevisions() {
    ...
    const allGainsAreZeroOrNegative = gain.litres <= 0 && gain.kwh <= 0 && gain.euros <= 0 && gain.co2 <= 0
    ...
    return (
        <div className="container fade-in">
            <button className="nav-button" onClick={() => navigate(`/user/${id}`)}>
                ← Retour à la page utilisateur
            </button>

            <div className="card">
                <h1>🔮 Prévisions pour {userName}</h1>
                ...
                <div className="prediction-section">
                    <p style={{ fontSize: "1.15rem", marginBottom: "1rem" }}>
                        <b>Un adulte français prend en moyenne 6 à 7 douches par semaine, soit environ 338 douches par an.</b>
                        <br />
                ...
                {/* 1. Eau */}
                <h2>💧 Prévision de la consommation d'eau sur un an</h2>
                {renderChart(
                    previsionUser.litres,
                    previsionOMS.litres,
                    "Consommation d'eau (L)",
                    "Litres",
                    "Votre consommation d'eau",
                    "Objectif OMS (5 min/douche)",
                    "#00bfff",
                    "#34c759",
                )}

                {/* 2. Energie */}
                <h2>⚡ Prévision de la consommation d'énergie sur un an</h2>
                {renderChart(
                    previsionUser.kwh,
                    previsionOMS.kwh,
                ...
                {/* 3. Coût */}
                <h2>💶 Prévision du coût des douches sur un an</h2>
                {renderChart(
                    previsionUser.euros,
                    previsionOMS.euros,
                    "Coût (€)",
                ...
                {/* 4. CO2 */}
                <h2>🌍 Prévision de l'empreinte carbone sur un an</h2>
                {renderChart(
                    previsionUser.co2,
                    previsionOMS.co2,
                    "Empreinte carbone (kgCO₂)",
                    "kgCO₂",
